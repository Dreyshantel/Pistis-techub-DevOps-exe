...
This documentation is designed to walk you through how to use Git to track your progress, manage changes, and revert a project back to the previous version if needed in cases of bugs. To proceed with the project you will have to clone the simple-form repository using the `git clone` command.
```
...
Once this is done, you will switch into the simple-form directory to stage the form files. Afterwards, you'll  place the client HTML(form.html) and CSS(style.css) form into the working directory..
``` 
# HTML file
<!DOCTYPE html>
<html lang="en">
...
Now that the files are in the working directory, you can track the current git status using `git status`, add both files to the staging area using the following command `git add form.html style.css`, and commit your initial version into your local git repository `git commit -m "message"`.
<img width="1207" height="382" alt="image" src="https://github.com/user-attachments/assets/657b98d6-9584-4769-9217-caae187714b6" />

Push the initial commit to the remote repository. `git push origin main"
...
You want add some new features to the form. so you modify the form.html to include an additional field for phone number and add styling to the style.css file. After making these changes, stage and commit the updated files, and push them to your remote repository.
...
After working on several features and fixes, view the git history to check all changes you have made so far.
...
You noticed the new changes you made introduced bugs into the form submission process. What to do in this situation is to revert form.html file back to the last working version. First, find the commit hash of the last working version using `git log` then run  `git checkout <commit hash> --form.html` 
...
If you accidentally made some unwanted changes to the style.css file that you haven't commited yet. you can discard these local changes and revert it to the previous working file using `git checkout -- style.css` command.
